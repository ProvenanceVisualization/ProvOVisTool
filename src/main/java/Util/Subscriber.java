...
public class Subscriber extends JedisPubSub {
    ...
    public void onMessage(String channel, String message) {
        ...
        FilterUtil filterUtil = new FilterUtil(myNet, myNet.getDefaultNodeTable(), this.adapter);
        ...
        if(panel.getEnhancedVersionOfBDM().getDoesFilterApplied()){
            ...
            for(String nodeId : panel.getEnhancedVersionOfBDM().getSelectedNodeIdList()){
                try {
                    nodesToBeShownOnly.addAll(panel.getEnhancedVersionOfBDM().getBackwardProvenance(nodeId, panel.getEnhancedVersionOfBDM().getStateCurrent(), new ArrayList<>(),false));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            ...
            panel.getNetworkViewOrganizer().showOnly(nodesToBeShownOnly,
                    new FilterUtil(adapter.getCyApplicationManager().getCurrentNetwork(), adapter.getCyApplicationManager().getCurrentTable(), this.adapter));
        }
