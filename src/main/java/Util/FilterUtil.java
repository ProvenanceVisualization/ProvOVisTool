...
public class FilterUtil {
    ...
    public FilterUtil(CyNetwork network, CyTable table, CySwingAppAdapter adapter){
        // Initializations
    ...
    public String findNodeTypeByName(String type, String name){
        String nodeType;
        ArrayList<CyNode> nodeList = getAllNodes();
        ...
        while(control == false && nodeIndex < nodeList.size())
        {
            node = nodeList.get(nodeIndex);
            if(findNodeName(node).equals(name))
            {
               control = true;
            }
            nodeIndex++;
    ...
    public boolean isEdgeTypeAssociation(String edgeType)
    {

        if(edgeType.equals("wasAssociatedWith"))
            return true;
    ...
    // This method is for finding node type of a node
    public String findNodeName(CyNode node){
        String nodeName;

        // Gets related row for a node and finds value which is in the "nodeType" column
        nodeName=network.getRow(node).get("name",String.class);

        return nodeName;
    }

    // This method is for finding node type of a node
    public String findNodeId(CyNode node){
        String nodeName;
        // Gets related row for a node and finds value which is in the "nodeType" column
        nodeName=network.getRow(node).get("id",String.class);
        return nodeName;
    }
