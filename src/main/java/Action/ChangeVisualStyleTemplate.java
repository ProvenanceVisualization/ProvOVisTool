...
public class ChangeVisualStyleTemplate{
    ...
    public void changeVisualStyle(Paint agentColor, Paint activityColor, Paint entityColor)
    {
        CyApplicationManager manager = this.adapter.getCyApplicationManager();
        CyNetworkView networkView = manager.getCurrentNetworkView();
        CyNetwork network = networkView.getModel();
        ...
        for(CyNode node: agents) {
            networkView.getNodeView(node).setVisualProperty(BasicVisualLexicon.NODE_FILL_COLOR, agentColor);
        }

        for(CyNode node: entities) {
            networkView.getNodeView(node).setVisualProperty(BasicVisualLexicon.NODE_FILL_COLOR, entityColor);
        }

        for(CyNode node: activities) {
            networkView.getNodeView(node).setVisualProperty(BasicVisualLexicon.NODE_FILL_COLOR, activityColor);
        }
